...
export function AdminDocs({ onClose }: { onClose: () => void }) {
  ...
  //  capacityRule = descriptif ; la règle « ≤ niveau du porteur » est appliquée d'office
          ...
          <ul className="adm-doc-list">
            ...
            <li><code>{`{ "kind": "grant-skill", "skillId": "heroique", "value": "défense", "precision": "hache" }`}</code> - octroie une compétence à la cible ; <code>value</code> (optionnel) précise le domaine d'une compétence « à valeur » (ex. Héroïque « défense »), <code>precision</code> (optionnel) une précision libre affichée à côté. Sur le profil, les valeurs d'une même compétence fusionnent (« Héroïque objectif et défense »).</li>
