...
export function AdminDocs({ onClose }: { onClose: () => void }) {
  ...
  //  capacityRule = descriptif ; la règle « ≤ niveau du porteur » est appliquée d'office
          ...
          <ul className="adm-doc-list">
            ...
            <li><code>{`{ "kind": "grant-skill", "skillId": "heroique", "value": "défense", "precision": "hache" }`}</code> - octroie une compétence à la cible ; <code>value</code> (optionnel) précise le domaine d'une compétence « à valeur » (ex. Héroïque « défense »), <code>precision</code> (optionnel) une précision libre affichée à côté. Sur le profil, les valeurs d'une même compétence fusionnent (« Héroïque objectif et défense »). <code>incrementIfPresent</code> (optionnel) : si la cible possède déjà la compétence nativement, sa valeur est <em>augmentée</em> de ce nombre au lieu d'octroyer <code>value</code> (ex. Symbiose « Moringa 3 ou +2 si déjà connue » ; Khépesh « Brutalité 1 ou +1 »).</li>
            <li><code>{`{ "kind": "grant-spell-choice", "magicWayId": "osteomancie", "count": 1 }`}</code> - le porteur connaît <code>count</code> sort(s) <strong>de son choix</strong>, pris dans la voie indiquée et/ou dans la liste <code>spellIds</code>. Sans grimoire, sans page et sans niveau consommé : ces sorts s'ajoutent à ce que la figurine achète par ailleurs. Le prix en Kouronnes du sort reste dû ; un sort réservé n'apparaît que chez qui y a droit ; un sort ne peut être connu qu'une fois. Ex. Demi-soeur : 1 sort d'Ostéomancie ; Vouge de Moringa : 3 sorts d'Adansonia.</li>
            <li><code>{`{ "kind": "spell-pages", "amount": 3 }`}</code> - ajoute des pages de sorts à la cible. <code>magicWayId</code> (optionnel) : les pages forment un <strong>pool dédié à cette voie</strong> (ex. Brassards d'Euthéria : 5 pages Adansonia + 5 pages shamanisme) ; sans lui, pages du <strong>budget général</strong> (toute voie accessible). Les sorts remplissent d'abord leur pool dédié, le surplus déborde sur le général.</li>
            ...
            <li><code>{`{ "kind": "stat-count", "stat": "t", "of": { "traits": ["dogon"] } }`}</code> - fixe une caractéristique au <strong>nombre de figurines</strong> correspondant à <code>of</code> dans la portée (ex. Instinct grégaire : T = nombre de Dogons) ; plancher = valeur de base imprimée du profil (absente = pas de plancher). <em>Affiché sur le profil.</em></li>
